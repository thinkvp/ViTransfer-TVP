...
- **Accounting and Sales nav bars scroll horizontally on mobile** — both section nav bars previously used `flex-wrap`, causing the nav pills to reflow into multiple rows on narrow viewports; they now use a single-row `overflow-x-auto` layout with `scrollbar-hide` so all tabs remain accessible by swiping; nav links gain `flex-shrink-0` to prevent compression; a `data-active` attribute is added to the active link and a `useEffect` calls `scrollIntoView` whenever the route changes, keeping the active tab visible without manual scrolling; a `.scrollbar-hide` utility class is added to `globals.css` to suppress the scrollbar while preserving scroll functionality across all browsers
