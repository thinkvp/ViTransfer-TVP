...
- **Emoji reactions on share-page comments** — agreeing with a note, or acknowledging one, meant writing "👍" as a reply, which pushed the thread longer for no added information. Comments and replies now carry a row of reaction pills with a fixed six-emoji set (👍 ❤️ 🎉 👀 🤔 🔥) behind an "add reaction" button. The set is a hardcoded allowlist in a new dependency-free `src/lib/comment-reactions.ts` shared by the server validator and the picker, so `emoji` is never free text and needs no sanitization downstream; pills render in allowlist order rather than by count, so one never jumps position when someone else reacts. Reactions are **attributable-only**: every reactor is an admin `User` or a project `ProjectRecipient`, and a share visitor who has typed a free-form name rather than picking themselves off the recipient list sees counts but gets a toast pointing at the name box instead of a reaction. That is what the two `@@unique` constraints key on, and a clearable session id would not have held that line. Counts and the viewer's own state are non-PII and go to every audience; reactor names (the pill tooltip) are admin-only, matching the existing zero-PII policy in `comment-sanitization.ts`. The API is one route with two idempotent verbs rather than a single toggle — `POST` adds, `DELETE ?emoji=` removes — so a double-click or a retry lands on the same end state instead of flipping it back; a `P2002` on insert is treated as success. It reuses the guard chain from `/api/comments/[id]` (rate limit → `hideFeedback` → `verifyProjectAccess` → guest reject → permission → `lockedAt`), with one deliberate difference: the reactor's recipient id is resolved **server-side**, because the sibling comment routes accept a body-supplied `recipientId` as a mere attribution label whereas here it is the uniqueness key, so trusting it would let a viewer inflate a count by cycling ids or fake another recipient's reaction. Admins are gated on `makeCommentsOnProjects` (participation) rather than `manageSharePageComments` (moderation), and nobody — admins included — can clear someone else's reaction. Locked comments (next version requested) refuse new reactions from share sessions, consistent with edit and delete; approving a video does **not** stop reactions, since it freezes new feedback rather than responses to feedback that already exists. Realtime is free: the route publishes the existing `comment` project event, so every open share page refetches without a new event type. A whole threaded list costs exactly one reaction query — `hydrateCommentReactions()` collects parent and reply ids together and rolls the rows up per comment — and the summaries ride into the response through a new sixth parameter on `sanitizeComment`. Replies needed no schema work (they are `Comment` rows with a `parentId`) but did need new UI: they had no bottom actions row at all, since edit and delete live up in the header beside the timestamp, so the pill row under a reply body is new markup at a compact scale with the add button dimmed until hover, to keep long threads calm. Adds the per-project **Allow clients to react to comments** switch (Feedback & Client Uploads) and its Settings → Default Project Settings counterpart — deliberately its own flag rather than folding into `allowClientDeleteComments`, which already doubles as the client *edit* permission and whose settings copy would have become inaccurate. Replies render as inset cards on a lighter surface instead of sharing the parent's, which let the per-reply `↳` arrow go — containment says the same thing louder — and the pill tooltip names who reacted ("You and Sarah Mitchell reacted") rather than repeating the emoji you are already pointing at. Naming reactors widened `reactorNames` from admin-only to `isAdmin || isAuthenticated`, matching the rule that already governs comment author names in `comment-sanitization.ts`; anonymous viewers on an open share still see counts only. One consequence worth stating: someone who only ever reacted and never commented is now named to that audience, where previously they were invisible. Finally, the viewer's chosen recipient id has to ride along on every comment-list fetch (`withCommentIdentity()` in a new `src/lib/comment-identity.ts`) because that identity lives in `sessionStorage`, not in the share token — miss one of the four fetch sites and reactions come back with `viewerReacted: false` everywhere, which renders fine and is silently wrong. Touches `src/lib/comment-reactions.ts`, `src/lib/comment-identity.ts`, `src/lib/comment-helpers.ts`, `src/lib/comment-sanitization.ts`, `src/app/api/comments/[id]/reactions/route.ts`, `src/app/api/comments/route.ts`, `src/app/api/comments/[id]/route.ts`, `src/app/api/share/[token]/comments/route.ts`, `src/app/api/share/[token]/route.ts`, `src/app/api/projects/route.ts`, `src/app/api/projects/[id]/route.ts`, `src/app/api/settings/route.ts`, `src/lib/validation.ts`, `src/components/CommentReactionBar.tsx`, `src/components/MessageBubble.tsx`, `src/components/CommentSection.tsx`, `src/hooks/useCommentManagement.ts`, `src/components/settings/VideoProcessingSettingsSection.tsx`, `src/app/admin/settings/page.tsx`, `src/app/admin/projects/[id]/settings/page.tsx`, `src/app/share/[token]/page.tsx`, `src/app/admin/projects/[id]/share/page.tsx`. **Schema migrations:** `20260813010000_comment_reactions`, `20260813010001_comment_reactions_default_setting`.

### Fixed

- **A multi-file ZIP download of a large selection stalled a few percent in and never finished** — selecting a folder of client uploads (16 files, 202 MB) and pressing Download reached ~3%, dropped to a couple of KB/s and sat there with a 25-hour ETA, having written nothing to disk. `downloadFilesAsZip()` started a `fetch` for **every** file up front and awaited all of them before handing the responses to client-zip, which writes members strictly in order. So fifteen responses sat open with nobody reading them: over HTTP/1.1 the first six filled the browser's per-host connection pool with bodies that can never finish (and therefore never free a slot, leaving the remaining requests undispatched and the `Promise.all` unresolved — a hard deadlock), and over HTTP/2 the stalled streams exhausted the session flow-control window until even the one stream being written could no longer receive data. Members are now fetched lazily as the archive writer reaches them, with a one-file lookahead, so at most two requests are ever open. The second half of the same bug: the File System Access save dialog was opened *after* those awaits, by which point the click's transient user activation had long expired, so Chrome refused it, the `console.warn` fallback path was taken silently and the entire archive was buffered in memory — no file appeared until the last byte arrived. The picker is now opened before the first `await` in the chain (it is the first statement of `runZipDownload`), so the ZIP streams to the chosen file as it is built; dismissing the dialog cancels without leaving a ghost transfer, and Firefox/Safari keep the in-memory Blob fallback as before. Touches `src/lib/download-zip-stream.ts`, `src/hooks/useDownloadTransfers.ts`. No schema migration.

- **A comment marker near the end of the video lost its avatar, leaving a bare reply-count bubble** — the last few percent of the timeline clamps a marker to `left: 100%` so the circle is not cut off at the edge. That leaves zero space to the right of the offset, and the marker button had no width of its own, so it sized shrink-to-fit against that zero. Tailwind's preflight puts `max-width: 100%` on `<img>`, which then resolved to `0px` and collapsed the avatar photo — and the button with it — to nothing; only the absolutely positioned reply badge, which does not depend on the parent's width, still painted. Authors on default initials were unaffected, because the initials `<div>` has an explicit width and no max-width, so the symptom looked random: it followed whoever had uploaded a profile picture. The marker button now carries the avatar's own width (`w-[30px] sm:w-[24px]`), which also fixes the same collapse mid-timeline on a narrow player, where the remaining space to the right of the offset can be a few pixels. Touches `src/components/VideoPlayer.tsx`. No schema migration.

- **Comment text showed raw HTML entities (`EHS &amp;amp; Sustainability`) wherever it was rendered as plain text** — comment bodies are stored as sanitized HTML, so a typed `&` is written as `&amp;` and `<` as `&lt;`. The threads themselves render through `dangerouslySetInnerHTML` and were always correct, but every surface that treats the content as text was printing the escaped form verbatim: the "Replying to…" preview above the composer (share page and admin share view), the timeline marker hover tooltip, the internal-comments reply preview, and kanban card comments and replies. A new `src/lib/comment-plain-text.ts` strips the permitted formatting tags and decodes entities in a single left-to-right pass — so a literal `&amp;lt;` comes back as the text `&lt;` instead of being decoded twice into `<` — and is DOM-free, so it behaves the same during SSR and on the server. The Feedback list's API-side preview moved onto it too, replacing a chain of `.replace()` calls that had that double-decode. Output is text-only by contract and is never handed to `dangerouslySetInnerHTML`. Touches `src/lib/comment-plain-text.ts`, `src/components/CommentInput.tsx`, `src/components/VideoPlayer.tsx`, `src/components/ProjectInternalComments.tsx`, `src/components/KanbanBoard.tsx`, `src/app/api/admin/feedback/route.ts`. No schema migration.

- **Collapsing a video version card lost the progress of an asset upload still running inside it** — the asset upload queue lived in component state inside `useAssetUploadQueue`, and collapsing the card unmounts `VideoAssetList` and with it the whole panel. The transfer itself kept going (nothing aborts the TUS/XHR request), but its queue row, percentage, speed and Pause/Cancel controls were gone, and re-expanding the card showed an empty panel with no sign an upload was in flight — the file simply appeared in the asset list minutes later. Worse, anything still sitting in the `queued` state never started at all, because the auto-start effect that drained the queue unmounted with the component. The queue now lives in a module-level store keyed by `videoId`, subscribed to via `useSyncExternalStore`, mirroring what `UploadManagerProvider` already does for video uploads: the panel is a view onto it rather than its owner, so unmounting the card no longer discards anything. Draining the queue moved out of a React effect into the store itself (`pump()`), so queued files keep starting while the card is closed; the before-unload guard became a single lazily-installed listener that checks every video's queue instead of one listener per mounted panel. Re-expanding the card re-opens the Upload Assets panel automatically when that version has uploads in flight, so the progress is visible again where it was left. The upload's completion callback (which refreshes the asset list) is dropped on unmount and re-registered on re-mount — the list re-fetches on mount anyway — while the transfer settings are kept so in-flight uploads carry on. Touches `src/hooks/useAssetUploadQueue.ts`, `src/components/VideoAssetList.tsx`. No schema migration.

## [2.4.8] - 2026-08-13

### Added

- **Supported file types for client uploads is now a setting, not a sentence** — the allowed types were a fixed list compiled into `fileUpload.ts`, described by a hand-written paragraph hanging off the "Allow clients to upload files to Projects" toggle that had already drifted from the code (it credited AI to Images and omitted MXF). Settings → Default Project Settings gains its own **Supported file types for client uploads** block: a tickbox per category — Images, Videos, Audio, Project Files, Documents, Fonts, Archives — each listing the extensions it actually covers, plus a comma-separated box for custom extensions (`fcpxml, blend, obj`) for formats the built-in list has never known about. MOGRT (Motion Graphics Template) joins Project Files as part of the same change — it is a ZIP container, so browsers report it as an archive MIME or as nothing, and the validator's extension fallback is what keeps it in Project Files when Archives is switched off. Unlike everything else in that section this is **system-wide and immediate**, not a default copied onto new projects: there is no per-project override, and changing it applies to every existing project at once. It governs client uploads only — admin uploads through the share page or the Uploads folder, and client voice notes, keep the full built-in list, matching how `allowClientUploadFiles` already behaves. The extension blocklist (EXE, JS, PS1, BAT, DLL and the rest) is not overridable: a blocked extension typed into the custom box fails the save with a message naming it, and is rejected by the validator even if one reaches the database another way. Every "supported types" string in the UI — the upload modal's list, the settings block's per-category hints — is now generated from `ALLOWED_FILE_TYPES` and the active policy rather than written out by hand, so the copy cannot drift again. Enforcement is server-side in all four client upload paths (share UPLOADS direct + S3 presign, comment attachment direct + S3 presign); the share page pre-checks a selection through a new public `GET /api/upload-policy` purely so a rejection surfaces before the bytes are sent. Adds `Settings.clientUploadCategories` and `Settings.clientUploadCustomExtensions` (JSON string arrays, like `defaultPreviewResolutions`); a null or unparseable value means every category is allowed, so an install that never touches the setting behaves exactly as it did before. The pure policy logic moved to a new dependency-free `src/lib/upload-policy.ts` (re-exported from `fileUpload.ts`) so it can be read by the settings cache without pulling Node built-ins into the Edge bundle. Touches `src/lib/upload-policy.ts`, `src/lib/fileUpload.ts`, `src/lib/settings.ts`, `src/app/api/settings/route.ts`, `src/app/api/upload-policy/route.ts`, `src/hooks/useUploadPolicy.ts`, `src/hooks/useCommentManagement.ts`, `src/components/FileUploadModal.tsx`, `src/components/settings/VideoProcessingSettingsSection.tsx`, `src/app/admin/settings/page.tsx`, `src/app/admin/projects/[id]/settings/page.tsx`, and the four upload routes. **Schema migration:** `20260813000000_client_upload_type_policy`.

- **The Uploads project type now states what clients can actually do with it** — whether the UPLOADS folder is visible to clients, and whether they can put anything in it, is decided by two switches in Feedback & Client Uploads, so the Project Type tickbox on its own told you nothing. Ticking **Uploads** now shows a one-line summary underneath it, derived live from those switches: "Not accessible by Clients." when Share Page Uploads is off, "Clients can access but not upload." when it is on without client file uploads, and "Clients can access and upload." when both are on. It updates as the switches are toggled, before saving, and runs on into the existing "Remove existing uploaded files to disable Uploads in this project." note as one paragraph rather than stacking two lines. Touches `src/app/admin/projects/[id]/settings/page.tsx`.

### Changed

- **Feedback & Client Uploads is reachable on a project with no Video type** — the section was gated on `enableVideos`, so an Uploads-only project could never open the two switches that decide whether its clients see or can add to the UPLOADS folder. It now appears when **either** Video or Uploads is enabled, in both the desktop section list and the mobile collapsible cards; the four comment/feedback rows inside it (Hide Feedback Section, Restrict Comments to Latest Version, Display Full Timecode, Allow clients to edit and delete client comments) stay gated on Video, so an Uploads-only project sees just the three upload settings. Video Processing remains Video-only, and the guard that bounces you back to Project Details when the active section disappears was split to match. Touches `src/app/admin/projects/[id]/settings/page.tsx`.

### Fixed

- **Every user's Edit User page showed the *signed-in admin's* own passkeys** — the PassKey Authentication section on `/admin/users/[id]` called `GET /api/auth/passkey/list`, which is self-scoped by design and ignores which user's page you are on, so the same passkey appeared on every user in the list and no user's actual passkeys were ever visible. The two buttons were worse than cosmetic: "Add PassKey" on someone else's page ran the WebAuthn ceremony against your own account (a passkey silently added to the admin, not to the user being edited), and the delete button removed one of your own passkeys while appearing to remove theirs. The list endpoint now accepts an optional `?userId=`, returning another user's passkeys only for a caller holding the Users menu + `manageUsers` action (self-listing keeps its existing settings/`changeSettings` gate); `DELETE /api/auth/passkey/[id]` accepts the same user-administrator permission as an alternative to the settings gate and passes `allowCrossUser` into `deletePasskey()`, which still verifies ownership and now logs an admin revocation as a `PASSKEY_DELETED` event at WARNING severity carrying `revokedByAdminId` — an unauthorised cross-user delete is still refused and logged CRITICAL as before. The page fetches by the edited user's id, and on someone else's page hides "Add PassKey" entirely (a passkey can only be created on its owner's device) while keeping revocation available so a lost or compromised authenticator can be removed. Touches `src/app/admin/users/[id]/page.tsx`, `src/app/api/auth/passkey/list/route.ts`, `src/app/api/auth/passkey/[id]/route.ts`, `src/lib/passkey.ts`. No schema migration.

- **The notifications dropdown could scroll sideways** — a long unbroken word in a notification title pushed the scroll container wider than the panel. The list now clips horizontally while keeping its vertical scroll. Touches `src/components/NotificationsBell.tsx`.

## [2.4.7] - 2026-08-08

### Added

- **The AI assistant can add to an existing project instead of only creating new ones** — a follow-up email about work already in the system had nowhere to go: the only outcome the Project pill could produce was a brand-new project. With the Project pill active the composer now offers a target — "Create a new project" (the default) or any project the user can see. Targeting one switches the run to an explicitly **additive** mode: the worker feeds the project's title, client, existing recipients and existing key dates to the model as `<current_project>`, a second system-prompt block instructs it to return only what this source *adds*, and the guards enforce the same thing independently — the client is pinned to the project's own, `proposedNewClient` is cleared, and any recipient or `(type, date)` key date already on the project is dropped with a note. Nothing is ever renamed, replaced or removed, so re-reading the same email simply produces an empty proposal. The review card becomes "Add to <project>" and applies recipients through `POST /api/projects/[id]/recipients`, key dates, a Gantt schedule only when the project has none, and the source attachments. `targetProjectId` is access-checked against the same visibility and assignment rules as the project's own routes. Touches `src/app/admin/assistant/page.tsx`, `src/app/api/admin/assistant/requests/route.ts`, `src/components/admin/assistant/ProjectProposalCard.tsx`, `src/lib/ai/prompts.ts`, `src/lib/ai/proposal-schemas.ts`, `src/worker/ai-assistant-processor.ts`. No schema migration.

- **The assistant can propose a reminder on a key date** — `ProjectKeyDate` has carried `reminderAt`/`reminderTargets` all along and the API accepted both, but the assistant never set one. The model now proposes `reminderDaysBefore` (an offset, never a datetime, so it cannot get the timezone maths wrong) when the source implies someone should be reminded ahead of the date; the guard clamps it to 0–90 and the review card renders an editable "Remind me N days before at 09:00" per row, addressed to the admin running the assistant. A computed time that has already passed drops the reminder and keeps the key date, rather than the API rejecting the whole row. Touches `src/lib/ai/proposal-schemas.ts`, `src/lib/ai/prompts.ts`, `src/components/admin/assistant/ProjectProposalCard.tsx`, `src/components/admin/assistant/helpers.ts`. No schema migration.

### Changed

- **The assistant no longer creates a Gantt schedule unless asked** — the review card's "Create Gantt schedule from the standard template" checkbox initialised to *ticked* whenever the model proposed a schedule, which it does for any brief mentioning a shoot date, so a full template schedule was built on almost every project created this way. The box now starts unticked. The anchor date is still pre-filled from the proposal and the prompt still works one out, so turning it on remains a single click. Nothing outside the assistant changed — the "View Gantt" button and `/admin/projects/[id]/gantt` are untouched. Touches `src/components/admin/assistant/ProjectProposalCard.tsx`.

- **A proposed client that matches nothing no longer silently pre-selects the first client in the list** — when the model returned neither a match nor a proposed new client, the review card fell back to `clients[0]`, which reads as a decision the assistant made but isn't one. It now leaves the picker empty and requires an explicit choice. Touches `src/components/admin/assistant/ClientPicker.tsx`.

### Fixed

- **Clients Overview and Projects Overview reported revenue on a basis that was neither cash nor accrual** — the Clients Overview leaderboard showed a single client for "Financial year to date" while six invoices had been paid inside that year. Both bottom charts were fed by `GET /api/admin/sales/projects-chart`, which returns only `CLOSED` projects and buckets each one by **`project.startDate`**, valuing it at the full non-VOID invoiced total. That matched no accounting convention: work started in June and paid in July landed in the prior financial year, a client whose project was still open was invisible no matter how much they had paid, and the figure shown was invoiced-in-full rather than anything received. The two charts also ignored `dashboardAmountsIncludeGst` entirely. Both now aggregate the same basis-aware unit as the Sales Overview chart and the FY sales KPI — `SalesSettings.dashboardReportingBasis`: under **Cash** a payment received (dated by `paymentDate`, skipping `excludeFromInvoiceBalance` mirrors), under **Accrual** an invoice raised (dated by `issueDate`, VOID worth nothing) — so all four dashboard revenue figures reconcile for a given period. Project counts are derived from the contributing documents' `projectId`, with an unlinked invoice or ad-hoc payment counting as one unit so it stays in the denominator of an average rather than disappearing. Each card now states the active basis under its title. The leaderboard takes client names from the existing `fetchClientOptions()` lookup, waiting for it rather than flashing "Unknown client". `GET /api/admin/sales/projects-chart` existed only to serve the old shape and had no other consumer, so it is removed along with its entry in the smoke test's VOID-exclusion check. Note that the Sales Dashboard basis is a **separate setting** from Accounting → Reporting Basis and defaults to Accrual where the accounting one defaults to Cash — an install that set only the accounting basis needs Sales → Settings → Sales Dashboard → Reporting basis changed too. Touches `src/components/sales/SalesDashboardCharts.tsx`, `src/app/admin/sales/page.tsx`, `src/app/admin/sales/settings/page.tsx`, `scripts/smoke-test.ts`. Removes `src/app/api/admin/sales/projects-chart/route.ts`. No schema migration.

- **The assistant created duplicate client contacts — same person, a different email address** — dropping in an email from a contact already on file (`nicholas.davis@…`) produced a *second* `ClientRecipient` under the same name with a subtly different address. Three things compounded. The model was never shown the contacts we already hold: `<clients>` in the prompt carried `id | name` and nothing else, so every recipient was re-derived from the source text and a reconstructed `first.last@domain` was as likely as the real thing. The prompt rule *"only output email addresses that literally appear in the brief"* was advisory only — `guardRecipients` checked the address against a regex and the own-team blocklist, never against the source, so an invented address passed straight through to the review card looking exactly like a real one. And the server deduped on exact email, so one wrong character meant a new contact row. All three are now closed: the prompt gains a `<known_contacts>` block listing each client's stored contacts (capped per client and overall) with a rule to copy those addresses character for character; the guard resolves a proposed contact against them by exact address and then by full normalised name, swapping in the stored address and id when the name matches — deliberately strict, requiring two name parts and an unambiguous single hit; and any address that survives without matching a stored contact must appear verbatim in the brief or an attachment or it is dropped with an explanation in the assumptions list, the same anti-hallucination backstop already used for reply URLs. A refine pass resends no source text and correctly skips the verbatim check rather than dropping everything. Touches `src/lib/ai/prompts.ts`, `src/lib/ai/proposal-schemas.ts`, `src/worker/ai-assistant-processor.ts`, `src/components/admin/assistant/ProjectProposalCard.tsx`. No schema migration.

- **Key dates from the assistant were always Full Day, discarding any time in the brief** — `ProjectKeyDate` has had `allDay`, `startTime` and `finishTime` since the baseline migration and the API validated all three, but the assistant's proposal schema carried only type, date and notes, and the review card posted a hard-coded `allDay: true`. A 07:30 call time in the source was simply lost. The schema now carries the times, the system prompt gained its first key-date rule at all (covering both the times and what the four types actually mean, which the model previously only saw as bare enum names), and the review card exposes an All-day checkbox plus Start/Finish pickers per row using the same component and clearing behaviour as the manual key-date editor. The guard validates `HH:MM`, clears a finish time that is not after the start, and normalises the `allDay: false` with no start time case to all-day — which also closes a latent inconsistency, since the calendar feed already rendered that combination as all-day while the database claimed otherwise. The row's Notes field, previously sent but never displayed, is now editable too. Touches `src/lib/ai/proposal-schemas.ts`, `src/lib/ai/prompts.ts`, `src/components/admin/assistant/ProjectProposalCard.tsx`. No schema migration.

- **Project recipients created with a project were orphaned from their client contact, and could wipe its display colour** — `POST /api/projects` inserted `ProjectRecipient` rows via `createMany` without ever setting `clientRecipientId`, unlike the single-add path in `addRecipient()` which resolves or creates the contact and links it. The same block then "synced" the contact's details with `displayColor: r.displayColor ?? null`, unconditionally overwriting an existing contact's colour with null whenever the caller supplied none — which the assistant always did. Contact resolution now happens before the recipients are created so the rows carry the link, an optional `clientRecipientId` is accepted per recipient and verified against the project's client, newly created contacts get a generated colour like the single-add path, and the sync only writes fields that were actually supplied. Touches `src/app/api/projects/route.ts`, `src/lib/validation.ts`. No schema migration.
...
- **A bank-linked asset account now reports a real balance in every date range, and the ledger page agrees with the Chart of Accounts column** — an ASSET account linked to a bank account (`coaAccountId`) had its balance computed two different ways, and both were wrong outside "All time". `/api/admin/accounting/accounts/balances` (the Chart of Accounts "Balance" column) added the bank account's `openingBalance` unconditionally but bounded the transactions on *both* ends, so any range that did not start at time zero silently dropped every movement before the window — on the default "This financial year" view the column read as the opening balance plus one year of movement, a figure that is neither a balance nor a period total. The individual ledger page had the mirror-image bug: it summed only the entries it lists and omitted the opening balance entirely, so the two screens disagreed by exactly that amount with no way to tell from the page which was right. Both now follow the rule the Balance Sheet already used: the balance is the position as at the *end* of the range, so the transaction sum is filtered on `to` alone. The ledger's entries endpoint returns `openingBalanceCents` (the opening balance plus every movement before `from`), `openingBalanceAsOf` and `openingBalanceIsBroughtForward`, and the ledger page and report drill-down dialog render it as an "Opening balance (date)" — or "Balance brought forward (as at date)" when a range is applied — above the period total, followed by a "Balance" row. The opening balance is also no longer counted in ranges that end before `openingBalanceDate`, which until now was stored and displayed but never used in any calculation. Unlinked accounts are unaffected (the fields come back `null` and no extra rows render). Touches `src/app/api/admin/accounting/accounts/balances/route.ts`, `src/app/api/admin/accounting/accounts/[id]/entries/route.ts`, `src/app/admin/accounting/chart-of-accounts/[id]/page.tsx`, `src/components/admin/accounting/AccountEntriesDialog.tsx`. No schema migration.

  Note on data: because the ledger page omitted the opening balance, entering the opening balance a second time as a manual journal on the bank's CoA account made that page look correct while inflating the Chart of Accounts balance by the same amount (the Balance Sheet was unaffected — it excludes linked bank accounts from its posted-lines pass). With the ledger page now sourcing the opening balance from the bank account itself, any such duplicate journal should be deleted.

- **A bank transaction can no longer be posted to its own bank account's Chart of Accounts account** — the balance counts every transaction once as raw cash, so coding one to the bank's own CoA account counted the same money twice, exactly the way a duplicate opening-balance journal did. (Money cannot move from an account to itself in any case.) The account is now hidden from both the posting and split-line pickers, and `POST /transactions/[id]/post` and `/split` reject it with an explanatory 400 rather than silently inflating the balance. Expense-type postings were never affected — they leave `accountId` unset and let the linked `Expense` own the account. Touches `src/app/admin/accounting/bank-accounts/page.tsx`, `src/app/api/admin/accounting/transactions/[id]/post/route.ts`, `src/app/api/admin/accounting/transactions/[id]/split/route.ts`. No schema migration.

- **`currentBalance` no longer silently reports the opening balance** — `bankAccountFromDb()` fell back to `openingBalance` when a caller had not computed a running balance, which `GET /api/admin/accounting/bank-accounts/[id]` never did. Nothing rendered the field, so the wrong figure was invisible rather than harmless. The derivation moved into a shared `withBankAccountBalances()` helper used by every route that returns a bank account, and the mapper's fallback is gone. Touches `src/lib/accounting/bank-balance.ts` (new), `src/lib/accounting/db-mappers.ts`, `src/app/api/admin/accounting/bank-accounts/route.ts`, `src/app/api/admin/accounting/bank-accounts/[id]/route.ts`. No schema migration.

### Changed

- **A bank account's opening balance may now be negative** — both bank-account endpoints validated `openingBalance` as `z.number().min(0)`, which made an overdrawn account or a credit card impossible to record. Touches `src/app/api/admin/accounting/bank-accounts/route.ts`, `src/app/api/admin/accounting/bank-accounts/[id]/route.ts`. No schema migration.

## [2.4.4] - 2026-07-30

### Fixed

- **Docker build no longer breaks when npm ships a new major** — the Dockerfile pinned node exactly (`node:24.13.0-alpine3.23`) but then ran `npm install -g npm@latest`. npm 12.0.2 requires node `^22.22.2 || ^24.15.0 || >=26.0.0`, so as soon as it was published every `docker compose build` failed with `EBADENGINE` in the `base-common` stage, taking both the app and worker images down. The global install is now pinned to the npm 11 major, which is what this project requires anyway — npm 10's `ci` rejects the nested `tinyglobby → picomatch` override in `package-lock.json` that npm 11 accepts. Touches `Dockerfile`. No schema migration.

- **Denied writes now always tell the user why, instead of failing silently** — a read-only Sales/Accounting role could edit a quote or invoice on screen and click Save with no feedback at all: the server correctly returned 403, but the quote page's `onSave` used `try { … } finally { … }` with no `catch`, so the rejection became an unhandled promise rejection — no toast, no state change, nothing. An audit found ~90 such catch-less write handlers across the Sales and Accounting UI, so fixing them one at a time would have left the next one silent. RBAC read-only denials now carry a `readOnly: true` marker, and `apiFetch` surfaces them as a toast centrally — every write path in the app is covered, including ones whose handlers swallow errors. The quote page also got the missing `catch` it should always have had (version-conflict reloads were equally silent before). Handlers that show their own error toast skip it for read-only denials via `isReadOnlyDenial()` so the message appears once, not twice. Touches `src/lib/api-client.ts`, `src/lib/rbac-api.ts`, `src/app/admin/sales/quotes/[id]/page.tsx`, `src/app/admin/sales/invoices/[id]/page.tsx`. No schema migration.

- **Creating a public share link for a quote or invoice was not write-gated** — `POST /api/sales/share` publishes a document to an unguessable public URL, but it lives outside `src/app/api/admin/` and so was skipped both by `check:rbac` and by the 2.4.3 write-gate sweep, leaving it on a menu-only gate. A read-only role could still publish documents externally. Now gated on `manageSales`. Touches `src/app/api/sales/share/route.ts`. No schema migration.

- **Sales showed raw cuids instead of client and project names for finance-only roles** — Sales stores clients and projects by id and resolved the names through `/api/clients` and `/api/projects`, which require the Clients and Projects menus. A role with only Sales/Accounting (e.g. an accountant) got a 403, `fetchClientOptions()` swallowed it and returned `[]`, and every invoice and quote fell back to displaying `clientNameById[clientId] ?? clientId` — a 25-character cuid. Generated PDFs were worse: they passed `clientName: undefined` and printed no client at all. A new `GET /api/admin/lookups` returns just the fields needed to label a document (client id/name/address, project id/title) to anyone with the Clients, Projects, Sales or Accounting menu, and `lookups.ts` falls back to it only when the primary endpoint is forbidden — so existing roles keep their current behaviour, including project-status visibility filtering. The endpoint deliberately excludes notes, phone, website, recipients and contact details. Touches `src/app/api/admin/lookups/route.ts` (new), `src/lib/sales/lookups.ts`, plus a new `requireApiAnyMenu` / `requireAnyMenuAccess` gate helper in `src/lib/auth.ts` / `src/lib/rbac-api.ts` (added to `scripts/check-rbac-gates.mjs`). No schema migration.
...
- **Bank CSV import no longer discards duplicate-flagged rows you deliberately re-selected** — the preview screen presents suspected duplicates as deselected-by-default suggestions, but the import endpoint then re-ran its own server-side deduplication over the selected rows and silently dropped any it considered duplicates. Re-ticking a flagged row had no effect: it was skipped anyway, and the "duplicates skipped" tally made it look intentional. When the caller passes an explicit `selectedIndices` set (which the preview flow always does), the import now inserts exactly what was selected — the duplicate decision has already been made by a human. Automatic deduplication still applies to the import-everything path, which has had no such review. Touches `src/app/api/admin/accounting/transactions/import/route.ts`. No schema migration.

- **Rows repeated within a single bank CSV are no longer flagged as duplicates** — duplicate detection treated a second row with the same date, amount and description as a duplicate of the first, even when nothing matching existed in the account yet. Statements legitimately list identical transactions (two same-day purchases from the same merchant, or two expenses the bank stamped with the same processing date), so this silently swallowed real expenses. Duplicate detection is now scoped to transactions that already exist in the bank account; repeats within the file are marked "×2" in the preview, stay selected, and all import. `deduplicateTransactions()` no longer folds already-inserted rows into its comparison set. Touches `src/lib/accounting/csv-parser.ts`, `src/app/api/admin/accounting/transactions/import/preview/route.ts`, `src/app/admin/accounting/bank-accounts/page.tsx`. No schema migration.

- **Import summary no longer reports unreadable rows as skipped duplicates** — the completion message read `BankImportBatch.rowCount`/`skippedCount`, where `skippedCount` was `duplicates + rows the parser could not read`. A CSV containing one unparseable line therefore reported "1 duplicate skipped" even when zero duplicates were found. The dialog now uses the response's separate `inserted`, `duplicates` and `skipped` counts and words them distinctly, and the batch's `skippedCount` additionally includes rows deselected in the preview so it accounts for every row in the file. Touches `src/app/api/admin/accounting/transactions/import/route.ts`, `src/app/admin/accounting/bank-accounts/page.tsx`. No schema migration.

- **Project rename triggered by a client reassignment now carries the new client ID through the confirm flow** — when you changed only a project's client (not its title), the rename-confirm endpoint received no `clientId` and therefore saw no storage-path delta, returning early — which left the follow-up settings save still seeing the old client and re-triggering the 202 rename-confirm modal in an infinite loop. The settings page now forwards the pending `clientId` to rename-confirm, and the endpoint updates `clientId` (plus `companyName`, matching the project PATCH route so the client-facing name and storage-path fallback can't be left pointing at the previous client) immediately, while leaving `storagePath` for the background worker to rebase on completion. Touches `src/app/admin/projects/[id]/settings/page.tsx`, `src/app/api/projects/[id]/rename-confirm/route.ts`. No schema migration.

- **Company logo now renders on public sales documents in S3 mode** — the public quote/invoice share page was the only place that routed `/api/branding/logo` through `next/image`. In S3 mode that endpoint 302s to a presigned R2 URL, and the Next image optimizer invokes the route in-process without following redirects — so it saw an empty body, logged `internal image response is empty for /api/branding/logo`, and dropped the logo from every client-facing quote and invoice. Local-storage mode was unaffected (that branch streams real bytes), which is why it went unnoticed. The image is now marked `unoptimized`, matching every other logo call site. Touches `src/app/sales/view/[token]/page.tsx`. No schema migration.

- **Invoice void guard no longer issues concurrent queries on a pinned transaction client** — the paid-amount check fetched the manual-payment and Stripe-payment aggregates with `Promise.all` inside an interactive transaction. Prisma pins a single pg connection for the duration of a transaction, so both queries landed on the same client at once, emitting `Calling client.query() when the client is already executing a query is deprecated` on every void. pg 8 serialises them internally so behaviour was correct, but the pattern is removed in pg@9. The two aggregates are now awaited sequentially. Touches `src/app/api/admin/sales/invoices/[id]/void/route.ts`. No schema migration.

### Changed

- **Match to Expense dialog handles long descriptions** — expense rows truncated the supplier + description to a single line, which made near-identical entries (repeat rideshare fares, recurring subscriptions) indistinguishable: the only difference, the booking reference, sat in the cut-off tail. The line now wraps to two lines (`line-clamp-2`), carries the full text as a hover tooltip, and the row grew right-hand padding so the amount column no longer sits flush against the scroll bar. The dialog widens to `max-w-3xl`, the list grows to `max-h-[50vh]`, and the amount is `whitespace-nowrap` so it can never wrap under pressure from a long description. Touches `src/app/admin/accounting/bank-accounts/page.tsx`. No schema migration.

- **`ShareFilesBrowser` memoization now includes `isAdmin` in dependency arrays** — three `useMemo`/`useEffect` hooks that compute selectable-file keys referenced `isAdmin` inside their callbacks (via `isSelectableDownloadableFile(file, isAdmin)`) but omitted it from the dependency arrays, producing React hooks exhaustive-deps lint warnings. Touches `src/components/ShareFilesBrowser.tsx`. No schema migration.
...
- **Bumped the `brace-expansion` transitive override from 5.0.6 to 5.0.7** — GHSA-3jxr-9vmj-r5cp (high-severity ReDoS via exponential-time expansion of consecutive non-expanding `{}` groups) was extended to cover 5.0.6, which the previous override pinned; the Docker image build's `npm audit --audit-level=high --omit=dev` gate therefore failed. Bumped to the patched 5.0.7 (reached only through `archiver → readdir-glob → minimatch`, the ZIP-builder path). Production audit is clean again. Touches `package.json`, `package-lock.json`. No schema migration.
