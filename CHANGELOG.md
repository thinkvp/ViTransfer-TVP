...
- **Delete previews & ZIPs for closed projects tool in Developer Tools** — a new "Delete previews & ZIPs for closed projects" section in Admin Settings → Developer Tools scans all CLOSED projects that still have preview files (480p, 720p, 1080p), timeline sprite directories, or album photo ZIP files on disk; a dry-run reports how many closed projects, videos, and files would be affected; a "Delete previews" button commits the deletion and clears the corresponding database fields so previews and ZIPs regenerate automatically if the project is ever re-opened
...
- **Storage provider abstraction** — new `storage-provider.ts` module abstracts download routing between Dropbox and local storage with a `resolveStorageDownloadTarget()` helper that transparently handles Dropbox temporary link generation with fallback to local streaming
...
- **Video player download button disabled during Dropbox upload** — the download button on share pages shows "Uploading…" while the video original is still being uploaded to Dropbox, preventing premature download attempts that would fail
