...
import { buildAccountingFilePath, writeAccountingFile, adjustAccountingFilesBytes } from '@/lib/accounting/file-storage'
...
// POST /api/admin/accounting/transactions/[id]/attachments — upload one or more files
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const rl = await rateLimit(
    ...
    'admin-accounting-txn-attachments-upload',
    authResult.id
  )
  if (rl) return rl
