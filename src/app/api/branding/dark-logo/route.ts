...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // If dark logo is not enabled, fall back to the regular logo
    if (!settings?.darkLogoEnabled) {
      return serveLogo(
        settings?.companyLogoMode ?? 'NONE',
        settings?.companyLogoPath ?? null,
        settings?.companyLogoUrl ?? null,
      )
    }

    // Serve dark logo if configured, otherwise fall back to regular
    const darkMode = settings.darkLogoMode
    const darkPath = settings.darkLogoPath
    const darkUrl = typeof settings.darkLogoUrl === 'string' ? settings.darkLogoUrl.trim() : ''

    if (darkMode === 'UPLOAD' && darkPath) {
      return serveUploadedLogo(darkPath)
    }
    if (darkMode === 'LINK' && darkUrl) {
      return proxyRemoteLogo(darkUrl)
    }

    // Dark logo enabled but not configured — fall back to regular
    return serveLogo(
      settings.companyLogoMode,
...
function serveLogo(
  ...
  const logoUrl = typeof url === 'string' ? url.trim() : ''

  if (mode === 'UPLOAD' && path) {
    return serveUploadedLogo(path)
  }
  if (mode === 'LINK' && logoUrl) {
    return proxyRemoteLogo(logoUrl)
  }
