...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (isS3Mode()) {
      ...
      // missing favicon returns a clean 404 rather than a 302 to a URL that then 403s.
      const exists = await s3FileExists(faviconPath)
      if (!exists) {
        return NextResponse.json({ error: 'Favicon not found' }, { status: 404 })
      }
      const presignedUrl = await s3GetPresignedStreamUrl(faviconPath, 3600, contentTypeFromPath(faviconPath))
