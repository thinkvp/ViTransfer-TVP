...
export default function SalesInvoicesPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('admin_sales_invoices_table_sort')
    if (!stored) return
    try {
      const parsed = JSON.parse(stored)
      const validKeys = new Set(['invoiceNumber', 'issueDate', 'dueDate', 'status', 'amount', 'client', 'project'])
  ...
  useEffect(() => {
    localStorage.setItem('admin_sales_invoices_table_sort', JSON.stringify({ key: tableSortKey, direction: tableSortDirection }))
  }, [tableSortKey, tableSortDirection])

  useEffect(() => {
    const stored = localStorage.getItem('admin_sales_invoices_records_per_page')
    if (!stored) return
  ...
  useEffect(() => {
    localStorage.setItem('admin_sales_invoices_records_per_page', String(recordsPerPage))
  }, [recordsPerPage])
                      ...
                      visibleInvoices.map((row) => (
                        ...
                        >
                          ...
                          <td className="px-3 py-2">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs ${invoiceStatusBadgeClass(row.effectiveStatus)}`}>
                              {invoiceStatusLabel(row.effectiveStatus)}
                            </span>
                          </td>
