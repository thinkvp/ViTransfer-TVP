                            ...
                            {activeTab !== 'UNMATCHED' && (
                              <div className="w-32 shrink-0 hidden sm:block">
                                {(() => {
                                  ...
                                  if (t.matchType === 'INVOICE_PAYMENT') return <span className="text-xs text-muted-foreground truncate block">Sales Income</span>
                              ...
                              {activeTab === 'UNMATCHED' ? (
                                ...
                                <div className="space-y-3 max-w-xl">
                                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-sm">
                                    ...
                                    {t.matchType !== 'SPLIT' && (() => { const acctId = t.accountId ?? t.expense?.accountId; const name = t.accountName ?? t.expense?.accountName; const code = acctId ? coaAccounts.find(a => a.id === acctId)?.code : undefined; if (acctId || t.matchType === 'INVOICE_PAYMENT') return <div><p className="text-xs text-muted-foreground">Account</p>{acctId && code ? <Link href={`/admin/accounting/chart-of-accounts/${code}`} className="text-sm text-primary hover:underline underline-offset-2">{name}</Link> : acctId ? <Link href={`/admin/accounting/chart-of-accounts/${acctId}`} className="text-sm text-primary hover:underline underline-offset-2">{name}</Link> : <p>Sales Income</p>}</div>; return null })()}
